...
export const Spawn: FC<SpawnProps> = ({ avatar, onSpawnFinish }) => {
  ...
  const usesMountAnimation = isValidFormat(
    animationProps?.onLoadedAnimation?.src
  );

  const [effectRunning, setEffectRunning] = React.useState(usesMountEffect);
  const [animationRunning, setAnimationRunning] =
    React.useState(usesMountAnimation);
  ...
  return (
    <>
      {usesMountEffect && (
        <SpawnEffect
          onLoadedEffect={
            animationProps.onLoadedEffect as {
              src: string;
              animationSrc?: string;
              loop?: number;
            }
      ...
      {usesMountAnimation && (
        <SpawnAnimation
          onLoadedAnimation={
            animationProps.onLoadedAnimation as { src: string; loop?: number }
          }
          avatar={avatar}
