...
import type { BackgroundProps } from "../interface/Background.interface"
import type { SectionProps } from "../interface/Section.interface"
...
const backgrounds = [
  {
    name: "Cosmic Nebula",
    class: "bg-gradient-to-b from-indigo-600 to-purple-600 via-pink-500 text-white",
  },
  {
    name: "Cyberpunk City",
    class: "bg-gradient-to-b from-blue-900 to-purple-800 via-pink-700 text-white",
  },
  {
...
const MobileSimulatorContext = createContext<MobileSimulatorContextType | null>(null)
...
export const MobileSimulatorProvider = ({
  ...
  const [currentBackground, setCurrentBackground] = useState<BackgroundProps>(backgrounds[0])
        ...
        ? [
            {
              ...
              title: "User Profile",
              icon: <FaUser />,
              content: <UserProfile />, // Replace with actual user profile component
            },
          ]
        : [
            {
  ...
  return (
    <MobileSimulatorContext.Provider
      value={{
        ...
        toggleScreen,
        screens,
        removeScreen: (id: number) => setActiveScreens((prev) => prev.filter((screenId) => screenId !== id)),
        closeAllScreens: () => setActiveScreens([]),
        ColorPickerAttrs,
      }}
    >
      {children}
      <SimulatorToggleButton showMobile={showMobile} setShowMobile={setShowMobile} />
      ...
      {showMobile && (
        <>
          <MobileSimulatorContainer
            ...
            screens={screens}
            removeScreen={(id) => setActiveScreens((prev) => prev.filter((screenId) => screenId !== id))}
            closeAllScreens={() => setActiveScreens([])}
