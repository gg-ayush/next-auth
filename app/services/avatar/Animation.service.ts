...
const loadBlobFile = async (blob: Blob): Promise<ClipWithType> => {
  try {
    ...
    return {
      group: (await fbxLoader.loadAsync(
...
export const loadAnimationClip = async (
  ...
  const animation =
    source instanceof Blob
      ? await loadBlobFile(source)
      : await loadPathFile(source);

  return animation.isFbx
    ? normaliseFbxAnimation(animation.group)
    : animation.group.animations[0];
};
